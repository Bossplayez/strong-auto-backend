...
const categoryTabs = [
  ...
  { label: 'Авто в Європі', value: 'EUROPE' },
];
...
function CatalogPage() {
  ...
  const resetFilters = () => {
    const current = getFiltersFromParams();
    // Keep category tab when resetting filters
    if (current.sourceRegion) {
      router.push(`/catalog?sourceRegion=${current.sourceRegion}`);
    } else {
      router.push('/catalog');
    }
  };
  ...
  const filters = getFiltersFromParams();
  const activeCategory = filters.sourceRegion || '';
  ...
  const filterSidebar = (
    <>
      ...
      <SelectField
        ...
        onChange={(v) => updateFilters({ model: v || undefined })}
      />
      ...
      <RangeField
        ...
        fromPlaceholder={`Від ${filterOptions?.yearRange.min ?? ''}`}
        toPlaceholder={`До ${filterOptions?.yearRange.max ?? ''}`}
        onFromChange={(v) => updateFilters({ yearFrom: v })}
        onToChange={(v) => updateFilters({ yearTo: v })}
      />
      ...
      <RangeField
        ...
        fromPlaceholder="Від"
        toPlaceholder="До"
        onFromChange={(v) => updateFilters({ priceFrom: v })}
        onToChange={(v) => updateFilters({ priceTo: v })}
      />
      ...
      <RangeField
        ...
        fromPlaceholder="Від"
        toPlaceholder="До"
        onFromChange={(v) => updateFilters({ mileageFrom: v })}
        onToChange={(v) => updateFilters({ mileageTo: v })}
      />
  ...
  return (
    <div className="pb-20 lg:pb-0">
      ...
      <div className="flex gap-4 px-4 sm:px-8 py-4 sm:py-5 max-w-container mx-auto">
        ...
        <aside
          className="hidden lg:block w-[240px] shrink-0 bg-white rounded-lg self-start sticky top-4 overflow-y-auto"
          style={{ padding: 18, maxHeight: 'calc(100vh - 100px)' }}
        >
          {filterSidebar}
        </aside>
        ...
        {mobileFiltersOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[80vh] overflow-y-auto p-5 animate-slide-up">
              ...
              <button
                onClick={() => setMobileFiltersOpen(false)}
                className="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-sm font-bold text-sm transition-colors mt-3"
              >
                Показати результати
              </button>
            </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2.5 mb-3.5">
            <div className="flex flex-wrap gap-1.5">
              {categoryTabs.map((tab) => (
                <button
                  key={tab.value}
                  onClick={() => updateFilters({ sourceRegion: (tab.value || undefined) as VehicleFiltersType['sourceRegion'] })}
          ...
          {/* Filter chips — mobile shows here too */}
          <FilterChips
            filters={filters}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border p-3 flex gap-3 lg:hidden z-40">
        <button className="flex-1 py-3 rounded-sm text-sm font-semibold text-fg border border-border-strong hover:bg-background transition-colors">
          Підписатись
        </button>
        <button
          onClick={() => setMobileFiltersOpen(true)}
          className="flex-1 py-3 rounded-sm text-sm font-semibold text-white bg-green-500 hover:bg-green-600 transition-colors flex items-center justify-center gap-2"
        >
